...
def __open_wrf_dataset_my_version(file, **kwargs):
    ...
    Updated Function from salem, the problem is that our time has no units (so i had to remove the check in the
    original function)
...
def create_new_dataset(ds):
    ...
    wrf = xr.Dataset(
        data_vars=dict(alb=(["time", "lat", "lon"], ds.alb.values), hfs=(["time", "lat", "lon"], ds.hfs.values),
            ...
            th=(["time", "height", "lat", "lon"], ds.th.values), tke=(["time", "height", "lat", "lon"], ds.tke.values),
            u=(["time", "height", "lat", "lon"], ds.u.values), v=(["time", "height", "lat", "lon"], ds.v.values),
...
def generate_filenames():
    ...
    hours_16 = [f"{hour:02d}{minute:02d}" for hour in range(0, 12) for minute in [0, 30]] + [
        "1200"]  # 0000, 0030, ..., 1200
    wrf_files_15 = [
        confg.wrf_folder + f"/WRF_ACINN_20171015/WRF_ACINN_20171015T1200Z_CAP02_3D_30min_1km_HCW_20171015T" + hour +
        "Z_regrid.nc"
        for hour in hours_15]
    wrf_files_16 = [
        confg.wrf_folder + f"/WRF_ACINN_20171016/WRF_ACINN_20171015T1200Z_CAP02_3D_30min_1km_HCW_20171016T" + hour +
        "Z_regrid.nc"
        for hour in hours_16]
...
def read_wrf_fixed_time(day=16, hour=12, min=0, variables=["p", "temp", "th", "rho",
                                                           "z"]):  # min_lat=46.5, max_lat=48.2, min_lon=9.2, max_lon=13
    ...
    filepath = (
                confg.wrf_folder + f"/WRF_ACINN_201710"
                                   f"{time.day:02d}/WRF_ACINN_20171015T1200Z_CAP02_3D_30min_1km_HCW_201710"
...
if __name__ == '__main__':
    ...
    # what would be better to take as var for model topography? terrain height hgt or geometric height z for
    # consistency  # with other models?! ~ 20m difference...  # wrf_extent.hgt.to_netcdf(confg.wrf_folder +
    # "/WRF_geometric_height_3dlowest_level.nc", mode="w", format="NETCDF4")  # wrf_tif = wrf_extent.rename({"lat":
    # "y", "lon": "x", "hgt": "band_data"})  # rename for tif export  # wrf_tif.rio.write_crs("EPSG:4326",
    # inplace=True)  # add WGS84-projection  # wrf_tif.isel(time=0).band_data.rio.to_raster(confg.wrf_folder +
    # "/WRF_geometric_height_3dlowest_level.tif")  # for xdem calc of slope I need .tif file
