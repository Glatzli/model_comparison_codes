...
def plot_stations_and_AROME_height_filled(df, model_name, ext_lat, ext_lon):
    ...
    for city_name, city_info in cities.items():
        ...
        if (ext_lat.start <= city_info["lat"] <= ext_lat.stop) and (
            ...
            if city_name == "Innsbruck Airport":
                ...
                lidar = ax.plot(city_info["lon"], city_info["lat"], marker='D', color='red', markersize=8,
                                transform=ccrs.PlateCarree(), label=f"{city_name} (Radiosonde)", linestyle='none')
            else:
                aws = ax.plot(city_info["lon"], city_info["lat"], marker='o', color='black', markersize=7)
        else:
            print(f"{city_name} is not inside lat-lon extent")
            continue
    ...
    for momma_name, momma_info in MOMMA_stations_PM.items():
        if (ext_lat.start <= momma_info["latitude"] <= ext_lat.stop) and (
            ...
            momma = ax.plot(momma_info["longitude"], momma_info["latitude"], marker='P', color="grey", markersize=8,
                    transform=ccrs.PlateCarree(), label=f"{momma_info['name']}", linestyle='none')

    for ec_name, ec_info in ec_station_names.items():
        ec = ax.plot(ec_info["lon"], ec_info["lat"], marker='s', markersize=6, color="k",  # , alpha=0.7
                transform=ccrs.PlateCarree(), label=f"{ec_info['name']}", linestyle='none')
    ...
    ax.legend(handles=[aws[0], ec[0], lidar[0], radiosonde[0], momma[0]], labels=["AWS", "EC station", "LIDAR", "Radiosonde", "T/RH Logger"])
    plt.tight_layout()
...
def plot_stations_detail(df, model_name, ext_lat, ext_lon):
    ...
    for city_name, city_info in cities.items():
        ...
        if (ext_lat.start <= city_info["lat"] <= ext_lat.stop) and (
            ...
            if city_name == "Innsbruck Airport":
                radiosonde = ax.plot(city_info["lon"], city_info["lat"], marker='*', color='orange', markersize=10,
                                     transform=ccrs.PlateCarree(), label=f"{city_name} (Radiosonde)",
                                     linestyle='none')
            elif city_name == 'Innsbruck Uni':
                lidar = ax.plot(city_info["lon"], city_info["lat"], marker='D', color='red', markersize=8,
                                transform=ccrs.PlateCarree(), label=f"{city_name} (Radiosonde)", linestyle='none')
    ...
    # mask = ~np.isin(np.arange(dataset.dims['nz']), exclude_indexes)
    sp_south = list(np.arange(10, 13)) + list(np.arange(34, 47))
    hobo_sp_north = hobo.isel(STATION_KEY=sp_north)
    hobo_sp_south = hobo.isel(STATION_KEY=sp_south)

    for i, hobo_name in enumerate(hobo_sp_north.hobo_id.values):  # plot slope profile north
        if (ext_lat.start  <= hobo.lat[i] <= ext_lat.stop) and (
                ext_lon.start  <= hobo.lon[i] <= ext_lon.stop):
            hobo_legend = ax.plot(hobo_sp_north.lon[i], hobo_sp_north.lat[i], marker='P', color="blue", markersize=8,
                                  transform=ccrs.PlateCarree(), label=f"{hobo.hobo_id}", linestyle='none')

    for i, hobo_name in enumerate(hobo_sp_north.hobo_id.values):
        if (ext_lat.start  <= hobo.lat[i] <= ext_lat.stop) and (
                ext_lon.start  <= hobo.lon[i] <= ext_lon.stop):
            hobo_legend = ax.plot(hobo_sp_north.lon[i], hobo_sp_north.lat[i], marker='P', color="lightblue", markersize=8,
                                  transform=ccrs.PlateCarree(), label=f"{hobo.hobo_id}", linestyle='none')
    ...
    # ax.legend(handles=[tawes[0], momma[0], ec[0]], labels=["AWS", "T/RH Logger", "EC station"], bbox_to_anchor=(-0.2, 0.5))  # for legend outside the plot
    ax.legend(handles=[ec[0], lidar[0], radiosonde[0], momma[0]],
              labels=["EC station", "LIDAR", "Radiosonde", "T/RH Logger"])  # aws[0], "AWS",
    plt.tight_layout()
...
def select_extent_around_LOWI(lat_degree=0.4, lon_degree=0.4):
    """function to select the extent that should be plotted around Innsbruck airport"""
    extent_lat = slice(station_files_zamg["LOWI"]["lat"] - lat_degree, station_files_zamg["LOWI"]["lat"] + lat_degree)
    extent_lon = slice(station_files_zamg["LOWI"]["lon"] - lon_degree, station_files_zamg["LOWI"]["lon"] + lon_degree)
    return ds.sel(nz=90, latitude=extent_lat, longitude=extent_lon), extent_lat, extent_lon


if __name__ == '__main__':

    pal = sequential_hcl("Terrain")  # terrain_hcl()
    pal2 = qualitative_hcl("Dark 3")
    levels_filled = np.arange(400, 3600, 400)
    levels_contour_lines = np.arange(100, 3600, 100)
    ...
    ds_large_extent, extent_lat_large, extent_lon_large = select_extent_around_LOWI(lat_degree=0.4, lon_degree=0.4)
    ds_small_extent, extent_lat_small, extent_lon_small = select_extent_around_LOWI(lat_degree=0.1, lon_degree=0.12)
    ...
    plot_stations_detail(ds_small_extent, "AROME", ext_lat=extent_lat_small, ext_lon=extent_lon_small)
