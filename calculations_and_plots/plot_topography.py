...
from confg import JSON_TIROL, TIROL_DEMFILE, cities, stations_ibox, MOMMA_stations_PM, dir_PLOTS, ec_station_names, \
    dem_file_hobos_extent, data_folder
    ...
    out_meta.update(
        {"driver": "GTiff", "height": out_img.shape[1], "width": out_img.shape[2], "transform": out_transform,
         "crs": pycrs.parse.from_epsg_code(
             4326).to_proj4()})  # hard-coded EPSG code for WGS84 cause data['init'] not available
...
def plot_stations_and_AROME_height(df, model_name, ext_lat, ext_lon):
    ...
    my_range = np.arange(min_ds_900, max_ds_900 + 1, dist)  # interval of 60m
    ...
    cs = ax.contour(df['lon'], df['lat'], df["z"], my_range, transform=ccrs.PlateCarree(),
    ...
    for city_name, city_info in cities.items():
        ...
        if (ext_lat.start <= city_info["lat"] <= ext_lat.stop) and (ext_lon.start <= city_info["lon"] <= ext_lon.stop):
    ...
    ibox_legend = Legend(ax, handles[station_counter:station_counter + len(stations_ibox)],
                         ...
                         title="Ibox stations")
    momma_legend = Legend(ax, handles[
        station_counter + len(stations_ibox):station_counter + len(stations_ibox) + len(MOMMA_stations_PM)], labels[
                              station_counter + len(stations_ibox):station_counter + len(stations_ibox) + len(
    ...
    ec_legend = Legend(ax, handles[station_counter + len(stations_ibox) + len(MOMMA_stations_PM):],
                       labels[station_counter + len(stations_ibox) + len(MOMMA_stations_PM):], loc="upper right",
                       title="EC flux stations")
...
def plot_stations_and_AROME_height_filled(df, model_name, ext_lat, ext_lon, ext_lat_small, ext_lon_small):
    ...
    :param df: xarray datarray with only the AROME model topography (selected lowes model level at a timestep and
    saved it)
    """
    fig, ax = plt.subplots(figsize=(10, 8), subplot_kw={'projection': ccrs.PlateCarree()})
    
    cs = ax.contourf(df['lon'], df['lat'], df["z"], levels=levels_filled, transform=ccrs.PlateCarree(), cmap=pal.cmap())
    ax.contour(df['lon'], df['lat'], df["z"], levels=levels_contour_lines, transform=ccrs.PlateCarree(), color="grey",
               linewidths=0.2)
    rect = Rectangle((ext_lon_small.start, ext_lat_small.start), ext_lon_small.stop - ext_lon_small.start,
                     ext_lat_small.stop - ext_lat_small.start, linewidth=1, edgecolor='black', facecolor='gray',
                     alpha=0.3)
    ax.add_patch(rect)
    ...
    # ax.legend(handles=[tawes[0], momma[0], ec[0]], labels=["AWS", "T/RH Logger", "EC station"], bbox_to_anchor=(
    # -0.2, 0.5))  # for legend outside the plot
    # ax.legend(handles=[momma[0], ec[0], lidar[0], radiosonde[0]], bbox_to_anchor=(-0.2, 1))  # , labels=["AWS",
    # "EC station", "LIDAR", "Radiosonde", "T/RH Logger"]
...
def plot_stations_detail(df, model_name, ext_lat, ext_lon):
    ...
    cs = ax.contourf(df['lon'], df['lat'], df["z"], levels=levels_filled, transform=ccrs.PlateCarree(), cmap=pal.cmap())
    ax.contour(df['lon'], df['lat'], df["z"], levels=levels_contour_lines, transform=ccrs.PlateCarree(), color=gray_hcl,
               linewidths=0.2)
    ...
    for i, hobo_name in enumerate(hobo.hobo_id.values):  # plot slope profile north
        if (ext_lat.start < hobo.lat[i] < ext_lat.stop) and (  # check if this hobo station is inside the extent
            ...
            if hobo_name in hobo_sp_north.hobo_id.values:
                ...
                sp_north = ax.plot(hobo.lon.values[i - 1], hobo.lat.values[i - 1], marker='P', color=darkred_hcl,
                                   markersize=8, transform=ccrs.PlateCarree(), label=r"SP${_N}$", linestyle='none')
...
def plot_lidar_and_Modelgrid_points(dataset, model_name, ext_lat, ext_lon):
    ...
    # Create contour lines with colors mapped from the terrain colormap
    cs = ax.contour(dataset['longitude'], dataset['latitude'], dataset["z"], my_range, transform=ccrs.PlateCarree(),
                    colors=[terrain_cmap(norm(level)) for level in my_range])
    ...
    for station_lidar in ["SL88", "SL75", "SL74", "SLXR142"]:
        ...
        ax.plot(lidar_station_df.attrs['lon'], lidar_station_df.attrs['lat'], marker='*', markersize=15, alpha=1,
    ...
    grid_legend = Legend(ax, handles[station_counter + len(station_lidar):],
                         labels[station_counter + len(station_lidar):], loc="lower right", title="LAT-LON GRID AROME")
...
def plot_height_diff_LOWI(ds, model_name):
    ...
    extent = [station_files_zamg["LOWI"]["lon"] - margin_x, station_files_zamg["LOWI"]["lon"] + margin_x,
              station_files_zamg["LOWI"]["lat"] - margin_y, station_files_zamg["LOWI"]["lat"] + margin_y]
    ...
    with rasterio.open(dem_file_hobos_extent) as clipped:
        ...
        image = ax.imshow(data, cmap="Greys", vmin=vmin, vmax=vmax, extent=[extent[0], extent[1], extent[2], extent[3]],
                          transform=ccrs.PlateCarree())
    ...
    ax.plot(station_files_zamg["LOWI"]["lon"], station_files_zamg["LOWI"]["lat"], marker='x', color='red',
            markersize=10, alpha=0.7, transform=ccrs.PlateCarree(), label="Position of Innsbruck airport",
...
def plot_topo_um_cartopy():
    ...
    cs = ax.contourf(um_lowest['grid_longitude'], um_lowest['grid_latitude'], um_lowest["z"], transform=um_orig_crs,
                     cmap=pal.cmap())
    plt.show()
...
if __name__ == '__main__':
    ...
    height_lowest = xr.open_dataset(confg.dir_AROME + "AROME_geopot_height_3dlowest_level.nc")  # open AROME
    # dataset lowest height
    # height_lowest = xr.open_dataset(
    #    confg.icon_folder_3D + "/ICON_geometric_height_3dlowest_level.nc")  # open ICON MODEL Height lowest level
    
    ds_small_extent, extent_lat_small, extent_lon_small = select_extent_around_LOWI(height_lowest, lat_degree=0.08,
                                                                                    lon_degree=0.08)
    ds_small_extent2, extent_lat_small2, extent_lon_small2 = select_extent_around_LOWI(height_lowest, lat_degree=0.1,
                                                                                       lon_degree=0.1)
    ds_large_extent, extent_lat_large, extent_lon_large = select_extent_around_LOWI(height_lowest, lat_degree=20,
                                                                                    lon_degree=20)  # orig 0.4 for
    
    # ---------------------------------------------------------------------------------------------------
    # topo overview plot: Needs to be changed due to new, short lat/lon names and for regridded models!
    # ---------------------------------------------------------------------------------------------------
