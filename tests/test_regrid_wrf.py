""" compare regridded dataset to original WRF output
...
ncfile0 = '/home/c707201/SeaDrive/My Libraries/My Library/div/gratzl_regrid/WRF_ACINN_20171015T1200Z_CAP02_3D_30min_1km_HCW_20171015T1200Z.nc'
ncfile1 = '/home/c707201/SeaDrive/My Libraries/My Library/div/gratzl_regrid/regridded/WRF_ACINN_20171015T1200Z_CAP02_3D_30min_1km_HCW_20171015T1200Z_regrid.nc'

# plots to check output
plotvars = ['hgt', 'th', 'w', 'u', 'v']
plotdir = '/home/c707201/SeaDrive/My Libraries/My Library/div/gratzl_regrid/test_plots'
...
def test_attrs(nc0, nc1):
    ...
    for attr in attrs0:
        if attr == 'stagger':
            if getattr(nc0, 'stagger') in ['X', 'Y']:
                assert getattr(nc1, 'stagger') == ''
            else:
                assert getattr(nc0, attr) == getattr(nc1, attr)
        elif attr == 'coordinates':
            if getattr(nc0, 'coordinates') in ['XLONG_U', 'XLONG_V']:
                assert getattr(nc1, 'coordinates') == 'XLONG'
            elif getattr(nc0, 'coordinates') in ['XLAT_U`', 'XLAT_V']:
                assert getattr(nc1, 'coordinates') == 'XLAT'
            else:
                assert getattr(nc0, attr) == getattr(nc1, attr)
        else:
            assert getattr(nc0, attr) == getattr(nc1, attr)
...
def test_dims_glob(nc0, nc1):
    ...
    # new dataset must contain 'south_north' and 'west_east'
    assert 'south_north' in dims1
    assert 'west_east' in dims1

    # other dimensions must be identical for both datasets
    for dim in dims1:
...
def test_shape(nc0, nc1):
    ...
    dims0 = nc0.shape
    dims1 = nc1.shape
    if 'south_north' in nc1.dimensions:
...
def test_plot(nc0, nc1, var):
    ...
    # lon/lat limits
    lonmin = np.min(nc1['lon'])
    lonmax = np.max(nc1['lon'])
    latmin = np.min(nc1['lat'])
    latmax = np.max(nc1['lat'])
    ...
    # 2D fields
    if len(np.squeeze(nc0[var]).shape) == 2:
        vmin = np.min(nc0[var])
        vmax = np.max(nc0[var])
        co = axs[0].contourf(np.squeeze(nc0['lon']), np.squeeze(nc0['lat']), 
                             np.squeeze(nc0[var]), vmin=vmin, vmax=vmax)
        axs[1].contourf(np.squeeze(nc1['lon']), np.squeeze(nc1['lat']), 
                         np.squeeze(nc1[var]), vmin=vmin, vmax=vmax, levels=co.levels)
    # 3D fields: plot lowest model level
    else:
        vmin = np.min(nc0[var][0,0,...])
        vmax = np.max(nc0[var][0,0,...])
        if nc0[var].stagger == 'X':
            data = np.squeeze(nc0[var][0,0,...])
            data = 0.5 * (data[:, :-1] + data[:, 1:])
            co = axs[0].contourf(np.squeeze(nc0['lon']), np.squeeze(nc0['lat']), 
                                 data, vmin=vmin, vmax=vmax)
        elif nc0[var].stagger == 'Y':
            data = np.squeeze(nc0[var][0,0,...])
            data = 0.5 * (data[:-1, :] + data[1:, :])
            co = axs[0].contourf(np.squeeze(nc0['lon']), np.squeeze(nc0['lat']), 
                                 data, vmin=vmin, vmax=vmax)
            ...
            co = axs[0].contourf(np.squeeze(nc0['lon']), np.squeeze(nc0['lat']), 
                                 np.squeeze(nc0[var][0,0,...]), vmin=vmin, vmax=vmax)
        axs[1].contourf(np.squeeze(nc1['lon']), np.squeeze(nc1['lat']), 
